...
dpg.create_context()
dpg.create_viewport(title='AI LAB', width=1580, height=720, resizable=False)
# dpg.show_debug()
# demo.show_demo()
dpg.setup_dearpygui()
dpg.show_viewport()
...
with dpg.viewport_menu_bar():
	with dpg.menu(label="File"):
		dpg.add_menu_item(label="Save", user_data="node_editor", callback=save_project)
		dpg.add_menu_item(label="Open", user_data="saving_file.pkl", callback=open_project)
# Training settings
with dpg.window(label="Training settings", tag="training_settings", width=400, height=720, pos=(1180, 0), no_move=True, no_close=True, show=True):
	training_window_id = dpg.last_item()
	values["dataset"] = dpg.add_input_text(label="Choose dataset", default_value="mnist", payload_type="dataset_choice", readonly=True, drop_callback=choose_dataset)
	### all datasets start
		...
		with dpg.group(horizontal=True, xoffset=200):
			with dpg.group():
				...
				with dpg.handler_registry():
					dpg.add_file_dialog(callback=load_dataset, tag="dataset_file_dialog", height=400, show=False, modal=True)
	...
	with dpg.group(label="Your dataset settings", tag="user_dataset_settings", show=False):
		dpg.add_checkbox(label="My dataset has training X and Y data", default_value=True, enabled=False, tag="yes_train", callback=lambda s, a: dpg.configure_item("train_columns", show=a))
		with dpg.group(tag="train_columns"):
			...
			dpg.add_checkbox(label="My dataset has test X and Y data", tag="yes_test", callback=lambda s, a: dpg.configure_item("test_columns", show=a))
	...
	dpg.add_separator()
	dpg.add_separator()
	values["loss"] = dpg.add_input_text(label="Choose loss function", default_value="BinaryCrossentropy", payload_type="loss_choice", readonly=True, drop_callback=lambda s, a: dpg.set_value(s, a["name"]))
	### all losses start
	...
	dpg.add_separator()
	dpg.add_separator()
	values["optimizer"] = dpg.add_input_text(label="Choose optimizer", default_value="Adam", payload_type="opt_choice", readonly=True, drop_callback=lambda s, a: dpg.set_value(s, a["name"]))
	...
	### all optimizers end
	values["lr"] = dpg.add_input_float(label="Learning rate", default_value=0.001, min_value=0, min_clamped=True, max_value=1, max_clamped=True)
	dpg.add_separator()
	dpg.add_separator()
	...
	dpg.add_separator()
	dpg.add_separator()
	dpg.add_button(label="Start training", width=385, height=25, user_data=values, callback=lambda s, a, u: start_training(s, a, u))
# NN model editor
with dpg.window(label="NN model editor", width=900, height=720, pos=(280, 0), no_move=True, no_resize=True, no_close=True, no_scrollbar=True, no_scroll_with_mouse=True):
	with dpg.group(horizontal=True, width=880, height=670, payload_type="nn_editor", drop_callback=lambda s, a: adding_layer(a['parent'], a['name'], a['type'])):
		with dpg.node_editor(tag="node_editor", callback=link_callback, delink_callback=delink_callback, minimap=True, minimap_location=dpg.mvNodeMiniMap_Location_BottomRight) as node_editor:
		...
		with dpg.group(horizontal=True, xoffset=200):
			with dpg.group():
				for element in elements["layers"]:
					...
					dpg.add_drag_payload(
						parent=dpg.last_item(),
						drag_data={"parent": node_editor_id, "name": dpg.get_item_label(dpg.last_item()), "type": element[1]},
